...
const BCV_HISTORY_MARKET = "reference";

async function fetchHistoricalRate(apiKey, market, date) {
  const response = await fetch(
    `${COTIZAVE_BASE}/v1/fx/rates/${market}/history?date=${date}`,
    {
      headers: {
        Accept: "application/json",
        "X-API-Key": apiKey,
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.message || `Historical rate unavailable for ${market}`);
  }

  if (data.rate == null) {
    throw new Error(`Historical rate unavailable for ${market}`);
  }

  return {
    rate: ceilRate(data.rate),
    updatedAt: data.updated_at ?? data.captured_at ?? null,
    market: data.market ?? market,
  };
}

async function fetchUsdtHistoricalRate(apiKey, date) {
  for (const market of USDT_MARKET_PRIORITY) {
    try {
      const result = await fetchHistoricalRate(apiKey, market, date);
      return {
        usdt: result.rate,
        usdtSource: market,
        usdtUpdatedAt: result.updatedAt,
      };
    } catch {
      continue;
    }
...
async function fetchRatesForDate(date) {
  ...
  if (date === todayDateString()) {
    ...
    return { ...live, rateDate: date };
  }

  const apiKey = process.env.COTIZAVE_API_KEY?.trim();
  if (!apiKey) {
    throw new Error("Historical rates require COTIZAVE_API_KEY");
  }

  const [bcv, usdt] = await Promise.all([
    fetchHistoricalRate(apiKey, BCV_HISTORY_MARKET, date),
    fetchUsdtHistoricalRate(apiKey, date),
  ]);

  return {
    usdBcv: bcv.rate,
    usdBcvUpdatedAt: bcv.updatedAt,
    usdt: usdt.usdt,
    usdtSource: usdt.usdtSource,
    usdtUpdatedAt: usdt.usdtUpdatedAt,
    fetchedAt: bcv.updatedAt,
    rateDate: date,
    source: "cotizave_history",
  };
}

module.exports = { fetchLiveRates, fetchRatesForDate };
